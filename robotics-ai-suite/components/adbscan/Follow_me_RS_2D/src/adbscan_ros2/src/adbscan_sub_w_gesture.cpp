...
private:
  ...
  void topic_3D_callback(const sensor_msgs::msg::PointCloud2::SharedPtr _msg)
  {
    ...
    if (result == 0) {
      ...
      if (Verbose_) {
        cout << "[RS state] motor=" << this->motor_initiated_ << " found=" << this->target_found
             << " gesture=" << this->gesture_category_ << " new_tgt=(" << new_target_loc.x << ","
             << new_target_loc.y << "," << new_target_loc.z << ")" << " tgt=(" << this->target_loc.x
             << "," << this->target_loc.y << ")" << endl;
      }
