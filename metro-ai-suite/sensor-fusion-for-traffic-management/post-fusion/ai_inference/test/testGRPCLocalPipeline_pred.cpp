...
std::unordered_map<std::string, int> CLASS_TO_ID = {{"car", 0}, {"truck", 1}, {"bus", 2}, {"motorcycle", 3}, {"bicycle", 4}, {"person", 5}};
            ...
            while (stream->Read(&reply)) {
                ...
                if (reply.status() == 0) {
                    ...
                    // }
                        ...
                        std::string filePath = predDir + "/" + fileName;
                        FILE* fp = fopen(filePath.c_str(), "w");
                        if (jsonMessage.count("roi_info") > 0) {
                            boost::property_tree::ptree roi_info = jsonMessage.get_child("roi_info");
                            for (auto roi_i = roi_info.begin(); roi_i != roi_info.end(); ++roi_i) {
                                auto roi_item = roi_i->second;
                                ...
                                if (x == 0 && y == 0 && w == 0 && h == 0) {
                                    ...
                                    fprintf(fp, "%d %f %d %d %d %d\n", CLASS_TO_ID[roi_class_name], roi_score, x, y, w, h);
                                }
                            }
                        }
                        fclose(fp);
...
int main(int argc, char **argv)
{
    try {
        ...
        if (argc < 9 || argc > 12) {
            std::cerr << "Usage: testGRPCLocalPipeline <host> <port> <json_file> <total_stream_num> <repeats> <data_path> <media_type> <pred_dir> [<pipeline_repeats>] "
        ...
        for (unsigned i = 0; i < threadNum; ++i) {
            ...
            std::thread t(workload, std::ref(host), std::ref(port), std::ref(contents), std::ref(dataPath), repeats, mediaType, i, crossStreamNum, warmupFlag,
                          pipeline_repeats, reportFileName, predDir);
