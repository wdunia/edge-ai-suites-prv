            ...
            while (stream->Read(&reply)) {
                ...
                if (reply.status() == 0) {
                    ...
                    if (jsonMessage.get<std::string>("Type", "") == "PerformanceData") {
                        ...
                        if (jsonMessage.count("video_latency2") > 0) {
                            latency = jsonMessage.get<double>("video_latency2");
                            g_video_latency_sum += latency;
                            ++g_video_latency_count;
                        }
                        if (jsonMessage.count("video_latency3") > 0) {
                            latency = jsonMessage.get<double>("video_latency3");
                            g_video_latency_sum += latency;
                            ++g_video_latency_count;
                        }
                        if (jsonMessage.count("video_latency4") > 0) {
                            latency = jsonMessage.get<double>("video_latency4");
                            g_video_latency_sum += latency;
                            ++g_video_latency_count;
                        }
                    }
