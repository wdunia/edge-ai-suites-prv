...
DEFAULT_DRIVE_IDS = [0, 2, 3, 4, 5, 6, 7, 8, 9, 10, 18]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs='+',
        default=DEFAULT_DRIVE_IDS,
        metavar='ID',
        help='drive ids to convert. Default: %(default)s')
    args = parser.parse_args()
    return args

# Main processing loop
def main(kitti_360_folder, output_folder, drive_ids):
    for drive_id in drive_ids:
        ...
        if not os.path.isdir(img_folder):
            print(f"Skipping drive {drive_id}: missing image folder {img_folder}", flush=True)
            continue
        if not os.path.isfile(world_to_ego_file):
            print(f"Skipping drive {drive_id}: missing pose file {world_to_ego_file}", flush=True)
            continue
        if not os.path.isdir(velodyne_folder):
            print(f"Skipping drive {drive_id}: missing LiDAR folder {velodyne_folder}", flush=True)
            continue
...
if __name__ == "__main__":
    ...
    drive_id_list = list(dict.fromkeys(args.drive_ids))
    ...
    main(kitti_360_folder, output_folder, drive_id_list)
